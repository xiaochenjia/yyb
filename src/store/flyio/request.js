...
// 接口请求封装函数
const handleRequest = (url = '', data = {}, isLoading = false) => {
  let _url = API[url] || ''

  return (flyConfig = {}, defaultTipConfig = {}) => {
    let flyio = Flyio.request(_url, data, {
      ...Config.flyConfig,
      headers: {'Content-Type': 'application/x-www-form-urlencoded', 'token': Store.token || '', 'cityId': Store.areaCode || ''},
      ...flyConfig
    })
    let tipConfig = {
      ...Config.reqConfig,
    ...
    return flyio.then(res => {
      ...
      if (res[Config.resSuccess.key] === Config.resSuccess.value) {
        Config.loading.loadingHide() // 当promise全部加载完成则隐藏loading
        return res
      } else {
