    ...
    <div class="search_kh">
      <div class="search_box">
        <div class="search_left"> <img src="http://ycdc-mkc.oss-cn-hangzhou.aliyuncs.com/wechat/sousuo_icon.png"/>
          <input placeholder="输入楼盘名称 " v-model="value" name="value" focus="true"/>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      selectProjectClick (item) {
        if (this.type === 'search') {
          ...
          this.$store.customerProjectId = item.id
          this.$store.customerProjectName = item.name
          this.$router.back()
        } else if (this.type === 'report') {
          this.$store.customerReport = item
          this.$router.back()
        }
      },
      initData () {
    ...
    onShow () {
      ...
      if (this.type === 'select' || this.type === 'report') {
        this.initSelect()
      }
    }
  }
...
<style scoped>
  ...
  .fang_list .fang_right{ float:left; width:2.1rem; height:1.6rem; margin-top:0.4rem; width:4.2rem; margin-left:0.4rem; position:relative;}
